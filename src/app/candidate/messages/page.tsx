            ...
            conversations.map((conv) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3.5 border-b border-[#E8DDC9] text-left transition-colors"
                  style={{ background: isActive ? KZ.cream2 : 'white' }}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {conv.job && (
                      <div className="text-xs text-[#6B5A4A] truncate">{conv.job.title}</div>
                    )}
                  </div>
                </button>
              )
        ...
        <div className="flex-1 flex flex-col bg-[#FFF7EE]">
          {!activeConvId ? (
            <div className="flex-1 flex items-center justify-center">
              <EmptyState
                title="Selectionne une conversation"
                description="Clique sur une conversation a gauche pour voir les messages."
            ...
            <>
              ...
              <div className="flex items-center gap-3 p-4 border-b border-[#E8DDC9] bg-white">
                {activeConv && (
                  <>
                    <Avatar name={otherUser(activeConv)?.full_name ?? 'KZ'} size={36} color={KZ.orangeSoft} />
                    <div>
                      <div className="text-sm font-bold text-[#1A1410]">{otherUser(activeConv)?.full_name}</div>
                      {activeConv.job && <div className="text-xs text-[#6B5A4A]">{activeConv.job.title}</div>}
                    </div>
                  </>
                )}
              </div>
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div
                        ...
                        style={{
                          background: isMe ? KZ.orange : KZ.paper,
                          color: KZ.ink,
                          boxShadow: isMe ? '2px 2px 0 #1A1410' : '2px 2px 0 #E8DDC9',
                        }}
                      >
                        <p>{msg.content}</p>
              ...
              <form onSubmit={handleSend} className="flex gap-2 p-4 border-t border-[#E8DDC9] bg-white">
                <input
                  ...
                  className="flex-1 h-11 px-4 bg-[#FFF7EE] border border-[#1A1410] rounded-lg text-sm outline-none"
                  placeholder="Ecrire un message..."
                />
                <Button type="submit" kind="primary" size="md" icon={<Send size={16} />}>
                  Envoyer
                </Button>
              </form>
            </>
